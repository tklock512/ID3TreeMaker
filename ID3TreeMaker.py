...
#function for comparing data and tree
def compare(tree, data, columns):
    right = 0
    t = tree
    unclassified = True
    for d in data:
        ...
        while unclassified:
            ...
            for c in columns:
                if c.name == attr:
                    index = c.column
            if index == None:
                print("Error: Test Data Variables and Training Data Variables do not match")
                sys.exit(3)
            num = d[index]
            if num == 0:
                if t.z:
                    if t.z.classification != None:
                        if d[-1] == t.z.classification:
                            right+=1
                        unclassified = False
                    else:
                        t = t.z
            if num == 1:
                if t.o:
                    if t.o.classification != None:
                        if d[-1] == t.o.classification:
                            right+= 1
                        unclassified = False
                    else:
                        t = t.o
            if num == 2:
                if t.t:
                    if t.t.classification != None:
                        if d[-1] == t.t.classification:
                            right += 1
                        unclassified = False
                    else:
                        t = t.t
...
trainRight = compare(t, trainDataArray, columns)

testRight = compare(t, testDataArray, testColumns)

print("Accuracy on training set (" + str(len(trainDataArray)) + " instances): " + str(round(100*trainRight/len(trainDataArray), 1)) + "%")
print()
print("Accuracy on testing set (" + str(len(testDataArray)) + " instances): " + str(round(100*testRight/len(testDataArray), 1)) + "%")
trainData.close()
